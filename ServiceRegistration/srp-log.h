...
// MARK: - Log Macros
    ...
    #define NO_CLOCK
        ...
        #define MACROSTRCAT(a,b,c) a##b##c
        // this used to use __FUNCTION__ - unclear what's the safe way concat in C preprocessor, likely more macros
        #define OPENLOG(progname, consolep) do { (void)(consolep); (void)progname; } while (0)
        #define ERROR(fmt, ...) fprintf(stderr, MACROSTRCAT("%s", fmt, "\n"), __FUNCTION__, ##__VA_ARGS__)
        #define INFO(fmt, ...)  fprintf(stderr, MACROSTRCAT("%s", fmt, "\n"), __FUNCTION__, ##__VA_ARGS__)
        #define FAULT(fmt, ...) fprintf(stderr, MACROSTRCAT("%s", fmt, "\n"), __FUNCTION__, ##__VA_ARGS__)
        #ifdef DEBUG_VERBOSE
            #ifdef IOLOOP_MACOS
                int get_num_fds(void);
            #endif // ifdef IOLOOP_MACOS
            #define DEBUG(fmt, ...) fprintf(stderr, MACROSTRCAT("%s", fmt, "\n"), __FUNCTION__, ##__VA_ARGS__)
